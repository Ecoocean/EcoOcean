...
import { saveFileToFireStorage, saveStringToFireBase } from './fileUploader';
...
export const PostgresPlugin = makeExtendSchemaPlugin((build) => {
  ...
  return {
    ...
    resolvers: {
        ...
        createPollutionReportExtend: async (parent, args, context, info) => {
          ...
          const document = /* GraphQL */ `
            mutation createPollutionReport(
              ...
              createPollutionReport(input: $input) {
                pollutionReport {
                  id
                }
              }
      ...
      Query: {
        exportCsv: async (parent, args, context, info) => {
          const document = /* GraphQL */ `
            query MyQuery {
              pollutionReports {
                nodes {
          ...
          const csv = await converter.json2csvAsync(
            data.pollutionReports.nodes
          );

          let date = new Date();
          let year = ('0' + (date.getFullYear() + 1)).slice(-2);
          let month = ('0' + (date.getMonth() + 1)).slice(-2);
          let day = ('0' + (date.getDate() + 1)).slice(-2);
          let filename = `Reports from : ${year}-${month}-${day}.csv`;
          console.log(filename);

          const url = await saveStringToFireBase(filename, csv);
          return url;
        },
